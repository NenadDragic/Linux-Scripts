# FileCount - Router - DashCam

Finds all files in `/volume1/DashCam/File-Count-DashCam/` modified today and displays their contents using `more`.
...
```bash
find /volume1/DashCam/File-Count-DashCam/ -maxdepth 1 -type f -newer /tmp/start_marker ! -newer /tmp/end_marker -exec basename {} \;
```
...
```bash
bash "FileCount - Router - DashCam.sh"
```

Replace `/volume1/DashCam/File-Count-DashCam/` with the target directory path if needed.
