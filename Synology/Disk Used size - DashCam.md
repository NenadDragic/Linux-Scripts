# Disk Used size - DashCam

Shows disk usage for `/volume1/DashCam/` up to 1 directory level deep, then prints the total.
...
| `grep -vE "total$|/volume1/DashCam/$"` | Remove the summary total and root path lines |
...
```bash
bash "Disk Used size - DashCam.sh"
```
