# Total Update Ubuntu (Desktop) Script

Runs a full system update pass on an Ubuntu Desktop machine: APT packages, Snap, Flatpak, firmware (fwupd), user-level Python pip packages, global npm packages, and the `locate` database — then reports whether a reboot is needed.
...
```console
chmod +x Total_Update_Ubuntu.sh
sudo bash Total_Update_Ubuntu.sh
```
...
Exits with an error if `$EUID -ne 0`. Initializes `REBOOT_NEEDED=false` and `START_TIME`. Reads `/etc/os-release` (if present) for `OS_NAME`/`OS_VERSION` and warns (non-fatal) if the detected `NAME` doesn't contain "ubuntu". Runs under `set -euo pipefail`, so most unguarded command failures abort the whole script.
...
- This is the Ubuntu **Desktop**-focused counterpart of [Total_Update_Ubuntu_Server.md](Total_Update_Ubuntu_Server.md) (drops Flatpak, which has no place on a headless box) and [Total_Update_RaspberryPi.md](Total_Update_RaspberryPi.md) (swaps `fwupd` for `rpi-eeprom-update`). It is a straight Ubuntu-labeled fork of [Total_Update_Debian.md](Total_Update_Debian.md), which remains available as the generic Debian/Ubuntu combined script.
