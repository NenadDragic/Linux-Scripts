...
- All four scripts operate over a fixed list or fixed base directory that is specific to the original author's setup: `Init_Git.sh` hardcodes a list of 15 repositories under the `NenadDragic` GitHub account, while `Pull_Git.sh` and `Status_Git.sh` default to iterating `~/git` — these need editing (repo list or `base_dir`) before reuse on another machine or account.
