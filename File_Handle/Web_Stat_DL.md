...
- The parent of `/home/admina/Documents/Web-Status` must exist — `rsync` creates the destination directory itself if it's missing, as long as its parent path is already there.
...
- Hardcoded values: remote user (`ElBosso` — a different user than the `admina` account used by the NAS-side scripts), remote host IP (`192.168.1.50`, the same Synology NAS referenced by `SMB.sh` in this folder), and the local destination path (`/home/admina/Documents/Web-Status`).
